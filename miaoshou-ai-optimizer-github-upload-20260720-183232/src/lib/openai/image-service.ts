...
export async function optimizeProductImage(input: ImageOptimizationInput): Promise<ImageOptimizationResult> {
  ...
  const validation = await validateOptimizedImage({ originalPath: input.originalPath, optimizedBuffer: buffer, expectedFormat: rules.output_format, title: input.title });
  // 不在一次点击里自动多轮重洗。中转站图片接口可能很慢，自动重试会让页面看起来一直卡在“洗图”。
  // 如果结果不满意，用户可以在审核页手动点“重新生成图片”。
...
function normalizeOpenAIImageError(error: unknown, model: string): Error {
  ...
  if (status === 524 || /524 status code|status code \\(no body\\)|status code \\(no body\\)/i.test(message)) {
